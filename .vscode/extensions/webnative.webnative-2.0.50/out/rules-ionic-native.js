...
exports.checkIonicNativePackages = checkIonicNativePackages;
const analyzer_1 = require("./analyzer");
const node_commands_1 = require("./node-commands");
const tip_1 = require("./tip");
function checkIonicNativePackages(packages, project) {
    ...
    for (const name of Object.keys(packages)) {
        if (name.startsWith('@ionic-native/')) {
            ...
            if (deprecatedPackages.includes(name)) {
                ...
                if ((0, analyzer_1.exists)(replacement)) {
                    project.recommendRemove(name, name, `You already have a newer version of this package installed (${replacement}) so ${name} can be uninstalled as it is not needed`);
                }
                else {
            ...
            if (plugin && !(0, analyzer_1.exists)(plugin)) {
                project.recommendRemove(name, name, `You have the typescript wrapper '${name}' installed but do not have the matching plugin '${plugin}' installed.`);
            }
        }
    }
}
function replacePackage(project, name, replacement) {
    project.add(new tip_1.Tip(name, 'Migrate to @awesome-cordova-plugins', tip_1.TipType.Idea, `@ionic-native migrated to @awesome-cordova-plugins in 2021. You can safely migrate from ${name} to ${replacement}`, (0, node_commands_1.npmInstall)(replacement) + ' && ' + (0, node_commands_1.npmUninstall)(name), `Replace ${name}`));
}
/**
...
//# sourceMappingURL=rules-ionic-native.js.map