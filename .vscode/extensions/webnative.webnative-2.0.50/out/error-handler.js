...
async function handleError(error, logs, folder) {
    ...
    if (error && error.startsWith('/bin/sh: npx')) {
        const zsh = '/bin/zsh';
        wn_tree_provider_1.exState.shell = zsh;
        wn_tree_provider_1.exState.context.workspaceState.update(context_variables_1.Context.shell, zsh);
...
function extractESBuildStyleError(errorText) {
    try {
        const lines = errorText.split('\n');
...
function extractNextJSErrorFrom(errorText) {
    try {
        const lines = (0, uncolor_1.uncolor)(errorText).split('\n');
...
function extractLintStyleError(errorText) {
    try {
        const lines = errorText.split('\n');
...
function extractViteErrorFrom2(txt) {
    try {
        const args = txt.split(':');
        const uri = args[0].split('(')[0];
        const error = args[1] + ' ' + args[2].trim();
        const pos = (0, utils_strings_1.getStringFrom)(txt, '(', ')').split(',');
...
function extractViteErrorFrom(fail, line) {
    try {
...
async function handleErrorLine(number, errors, folder) {
    ...
    const fixButton = (0, integrations_builder_1.hasBuilder)() ? fixTitle : undefined;
    const title = errors.length > 1 ? `Error ${number + 1} of ${errors.length}: ` : '';
    vscode_1.window.showErrorMessage(`${title}${errors[number].error}`, fixButton, prevButton, nextButton, 'Ok').then((result) => {
        if (result == 'Next') {
        ...
        if (result == fixTitle) {
            ...
            (0, logging_1.hideOutput)();
            (0, integrations_builder_1.chat)(wn_tree_provider_1.exState.projectRef.projectFolder(), undefined, undefined, prompt);
        }
    });
